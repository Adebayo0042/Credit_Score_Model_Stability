...
SHAP, a cooperative game theory approach, was used to explain feature contributions to predictions from our best-performing model. This method supports our goal of improving model transparency.
    ...
    if st.button("Perform Sensitivity Analysis"):
        st.subheader(f"Sobol Sensitivity Analysis: First order indices for {sda.most_stable_model} (Best Stable model)")
        model_explainer.sensitivity_analysis()
        st.success("Sensitivity Analysis Completed")


    if st.button("SHAP Explanations"):
        st.subheader(f"SHapley exPlanations Values: Explanations for {sda.most_stable_model}")
...
# Section 8: Conclusion
    ...
    st.write(f"In conclusion, this research advances credit scoring by developing a \
             ...
             The {sda.most_stable_model} showed the highest stability with a score of {sda.most_stable_model_score.round(2)} \
             and an accuracy of {sda.most_stable_model_acc_score.round(2)}. All models performed well, \
             with accuracies above {sda.min_acc.round(2)}. Model interpretability was enhanced through Sobol sensitivity analysis and SHAP values, \
             particularly for the {sda.most_stable_model} model. This deepened our understanding of feature impacts on predictions. \
             Overall, our methods proved effective in enhancing predictive performance and stability, with the  {sda.most_stable_model}\
             surpassing previous benchmarks. These approaches offer a valuable framework for future research in predictive \
